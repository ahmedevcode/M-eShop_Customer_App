...
class StateItem extends State<ProductDetail> with TickerProviderStateMixin {
          ...
          if (!error) {
            ...
            for (int i = 0;
              ...
              for (int variant = 0;
                ...
                for (int attribute = 0;
                    attribute <
                        widget.model!.attributeList![i].getIdList().length;
                    attribute++) {
                  if (variantAttributes[variant] ==
                      widget.model!.attributeList![i].getIdList()[attribute]) {
                    index = attribute;
                    break;
                  }
                }
  ...
  // Widget _buildSimpleAttribute(AttributeSelection attributeSelection) {
  //   return Column(
  //     crossAxisAlignment: CrossAxisAlignment.stretch,
  //     mainAxisSize: MainAxisSize.min,
  //     children: [
  //       Text(
  //         "${attributeSelection.oneAttributeList.name} : ${attributeSelection.oneAttributeList.value}",
  //         maxLines: 1,
  ...
  int getSelectedVariantIndex() {
    List<String> selectedAttributeIds =
        attributes.map((e) => e.selectedId).toList();
    int variantIndex = 0;
    ...
    for (int j = 0; j < widget.model!.prVarientList!.length; j++) {
      ...
      for (int i = 0; i < selectedAttributeIds.length; i++) {
        if (variant.attribute_value_ids!.contains(selectedAttributeIds[i])) {
          areAllThere = true;
        } else {
          areAllThere = false;
          break;
