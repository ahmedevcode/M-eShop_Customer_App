...
class DashboardPageState extends State<Dashboard>
  ...
  _getAppBar() {
    final appBar = AppBar(
      ...
      automaticallyImplyLeading: false,
      backgroundColor: Theme.of(context).colorScheme.white,
      title: Theme.of(context).colorScheme.white == const Color(0xffFFFFFF)
          ? SvgPicture.asset(
              DesignConfiguration.setSvgPath('titleicon'),
              height: 40,
            )
      ...
      actions: <Widget>[
        ...
        // Location Icon
        appbarActionIcon(() {
          // Define the behavior when the location icon is tapped
          _showCountryDialog(context); // Open dialog with current country
        }, 'location_icon'), // Ensure the correct icon path is provided
      ],
    );
...
// Function to show the dialog with the current country
    ...
    showDialog(
      ...
      builder: (BuildContext context) {
        return AlertDialog(
          ...
          content: Text('Syria'), // Display the current country here
          actions: [
            TextButton(
              child: Text('OK'),
              onPressed: () {
